...
func (qq *QuestionQuery) QueryOptions() *QuestionOptionQuery {
...
// QueryNextQuestionInv chains the current query on the "next_question_inv" edge.
func (qq *QuestionQuery) QueryNextQuestionInv() *QuestionQuery {
...
// QueryNextQuestion chains the current query on the "next_question" edge.
func (qq *QuestionQuery) QueryNextQuestion() *QuestionQuery {
	query := (&QuestionClient{config: qq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (qq *QuestionQuery) QueryPoll() *PollQuery {
	query := (&PollClient{config: qq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := qq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (qq *QuestionQuery) WithOptions(opts ...func(*QuestionOptionQuery)) *QuestionQuery {
	query := (&QuestionOptionClient{config: qq.config}).Query()
...
// WithNextQuestionInv tells the query-builder to eager-load the nodes that are connected to
// the "next_question_inv" edge. The optional arguments are used to configure the query builder of the edge.
func (qq *QuestionQuery) WithNextQuestionInv(opts ...func(*QuestionQuery)) *QuestionQuery {
	query := (&QuestionClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qq.withNextQuestionInv = query
	return qq
}

// WithNextQuestion tells the query-builder to eager-load the nodes that are connected to
// the "next_question" edge. The optional arguments are used to configure the query builder of the edge.
func (qq *QuestionQuery) WithNextQuestion(opts ...func(*QuestionQuery)) *QuestionQuery {
	query := (&QuestionClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	qq.withNextQuestion = query
	return qq
}
...
func (qq *QuestionQuery) WithPoll(opts ...func(*PollQuery)) *QuestionQuery {
	query := (&PollClient{config: qq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (qq *QuestionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Question, error) {
	...
	if query := qq.withOptions; query != nil {
		if err := qq.loadOptions(ctx, query, nodes,
			func(n *Question) { n.Edges.Options = []*QuestionOption{} },
			func(n *Question, e *QuestionOption) { n.Edges.Options = append(n.Edges.Options, e) }); err != nil {
			return nil, err
		}
	}
	if query := qq.withNextQuestionInv; query != nil {
		if err := qq.loadNextQuestionInv(ctx, query, nodes,
			func(n *Question) { n.Edges.NextQuestionInv = []*Question{} },
			func(n *Question, e *Question) { n.Edges.NextQuestionInv = append(n.Edges.NextQuestionInv, e) }); err != nil {
			return nil, err
		}
	}
	if query := qq.withNextQuestion; query != nil {
		if err := qq.loadNextQuestion(ctx, query, nodes, nil,
			func(n *Question, e *Question) { n.Edges.NextQuestion = e }); err != nil {
			return nil, err
		}
	}
...
func (qq *QuestionQuery) loadOptions(ctx context.Context, query *QuestionOptionQuery, nodes []*Question, init func(*Question), assign func(*Question, *QuestionOption)) error {
	...
	query.Where(predicate.QuestionOption(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(question.OptionsColumn), fks...))
	}))
...
func (qq *QuestionQuery) loadNextQuestionInv(ctx context.Context, query *QuestionQuery, nodes []*Question, init func(*Question), assign func(*Question, *Question)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Question)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Question(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(question.NextQuestionInvColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.question_next_question
		if fk == nil {
			return fmt.Errorf(`foreign-key "question_next_question" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "question_next_question" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (qq *QuestionQuery) loadNextQuestion(ctx context.Context, query *QuestionQuery, nodes []*Question, init func(*Question), assign func(*Question, *Question)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Question)
	for i := range nodes {
		if nodes[i].question_next_question == nil {
			continue
		}
		fk := *nodes[i].question_next_question
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "question_next_question" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (qq *QuestionQuery) loadPoll(ctx context.Context, query *PollQuery, nodes []*Question, init func(*Question), assign func(*Question, *Poll)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Question)
	for i := range nodes {
		fk := nodes[i].PollID
...
func (qq *QuestionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := qq.ctx.Fields; len(fields) > 0 {
		...
		if qq.withPoll != nil {
			_spec.Node.AddColumnOnce(question.FieldPollID)
		}
	}
