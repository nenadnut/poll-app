...
func (pq *PollQuery) QueryQuestions() *QuestionQuery {
	query := (&QuestionClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *PollQuery) WithQuestions(opts ...func(*QuestionQuery)) *PollQuery {
	query := (&QuestionClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PollQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Poll, error) {
	...
	if query := pq.withQuestions; query != nil {
...
func (pq *PollQuery) loadQuestions(ctx context.Context, query *QuestionQuery, nodes []*Poll, init func(*Poll), assign func(*Poll, *Question)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(question.FieldPollID)
	}
